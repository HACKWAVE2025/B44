...
# API Keys (Consider loading securely, e.g., from environment variables)
API_KEYS = [
    "AIzaSyBaVSh-3Ccecj_UbkHyPhp42nJMFYc5O9g",
    "AIzaSyDWLbAAi8He1CSPwjRYDIGTXGnUvMpMrQM",
    "AIzaSyBmfdBd2aD31Wo-SEVzw3Bj91Wa7JK0BPA",
    "AIzaSyD28McMJpbVwzzTBMZcXwfyMlTpMEAcKeg",
    "AIzaSyCkPtj82rwPSgMobdbGplsJuDBVEUVbmOk",
    "AIzaSyBaVSh-3Ccecj_UbkHyPhp42nJMFYc5O9g"
]
API_KEYS = API_KEYS[::-1] # Reverse for cycling

MODEL_NAME = "gemini-2.0-flash" # Updated model name

# Rate Limiting and Retries
RATE_LIMIT_DELAY_UPLOAD = 1  # seconds
RATE_LIMIT_DELAY_GENERATE = 2 # seconds
...
TEXT_CHUNK_CHAR_LIMIT = 900000 # Approx 1M tokens for Flash

# Prompts
FILE_ANALYSIS_PROMPT = """
**Analyze the data in the provided file ('{filename}') IN DETAIL using Markdown.**

**File Content Summary:**
Provide a concise overview of the file type and the general nature of its content.

**Detailed Analysis:**
*   **Key Information & Entities:** Identify and list the most important pieces of information, names, locations, organizations, dates, numerical values, or specific data points present in the file.
*   **Patterns & Relationships:** Describe any observable trends, correlations, distributions, or recurring patterns within the data. How do different data points or sections relate to each other?
*   **Themes & Topics:** Summarize the main subjects or themes represented by the data in the entire file.
*   **Potential Insights & Observations:** What interesting conclusions, anomalies, or questions arise from this file? Mention anything noteworthy or unexpected.
*   **Data Quality Notes (Optional):** Briefly comment on any apparent inconsistencies, missing values, formatting issues, or potential limitations observed in the file data.

**Format your entire response using Markdown.**
Use headings (`##`), bullet points (`*` or `-`), and bold text (`**text**`) for clarity.
Focus on analyzing the content of the *entire* file provided.
"""
...
*   Identify key information, entities, patterns, themes, etc., *within this chunk*.
...
def get_mime_type(filename):
    ...
    # ... (rest of the mime type mapping remains the same) ...
    ...
    return None # Let the API infer if unknown

# --- Main Analyzer Class ---

class FileAnalyzer:
    def __init__(self, session_id, api_keys_list, temp_base_folder="analysis_temp"):
        """Initializes the FileAnalyzer."""
        self.session_id = session_id
        self.api_keys = api_keys_list
        if not self.api_keys:
            raise ValueError("FileAnalyzer requires a list of API keys.")
        self.api_key_cycle = itertools.cycle(self.api_keys)
        self.current_api_key = None
        self.uploaded_api_file_names = [] # Stores API resource names (e.g., 'files/abc-123')
        ...
        if not self._rotate_api_key(): # Attempt initial configuration
            raise ConnectionError(f"Analyzer {self.session_id}: Failed to configure any initial API key.")

    # --- Public Methods ---

    def get_message_queue(self):
        """Returns the message queue for progress and results."""
        return self.message_queue

    def analyze_file(self, filepath):
        """
        Analyzes a single file, handling chunking, API calls, and cleanup.
        Sends progress/results/errors to the message queue.
        """
        filename = os.path.basename(filepath)
        logger.info(f"Analyzer {self.session_id}: Starting analysis task for '{filename}'")
        ...
        try:
            ...
            if should_chunk:
                ...
                try:
                    ...
                    if not chunk_files:
                        # Handle cases where splitting results in no files (e.g., empty input, single page PDF too large)
                        if os.path.exists(filepath) and os.path.getsize(filepath) < 50: # Arbitrary small size
                        ...
                        for i, chunk_filepath in enumerate(chunk_files):
                            ...
                            if upload_err:
                                ...
                                last_analysis_summary = f"[Chunk {chunk_num} upload failed: {upload_err}]" # Provide context for next potential chunk
                                if "Invalid API Key" in upload_err or "All API Keys Failed" in upload_err:
                                    if not self._rotate_api_key():
                                        ...
                                        break # Stop processing further chunks if all keys fail
                                continue # Try next chunk

                            # Wait for chunk to be active
                            ...
                            if wait_err:
                                ...
                                self._cleanup_specific_api_file(uploaded_chunk_obj.name) # Clean up the failed file
                                continue # Try next chunk

                            # Generate analysis for the active chunk
                            ...
                            if analysis_err:
                                ...
                                logger.error(f"Analyzer {self.session_id}: {error_msg}")
                                # Message queue put happens inside _generate_analysis_for_file on error
                                ...
                                if "Invalid API Key" in analysis_err or "All API Keys Failed" in analysis_err:
                                     if not self._rotate_api_key():
                                        ...
                                        logger.critical(f"Analyzer {self.session_id}: {final_status}")
                                        break # Stop processing further chunks if all keys fail
                                # Don't cleanup the API file here, _generate_analysis_for_file might retry
                            else:
                                successful_chunks += 1
                                ...
                                # Extract a summary for the next chunk's context (simple approach: take first N chars or lines)
                                # This could be improved by asking the LLM for a summary in the prompt.
                                summary_limit = 500 # Characters
                        ...
                        # Determine final status for chunked processing
                        if final_status == "UNKNOWN": # Only set if not already set by critical failure (like all keys failing)
                ...
                if upload_err:
                    ...
                    # Attempt key rotation if applicable
                    if "Invalid API Key" in upload_err or "All API Keys Failed" in upload_err:
                         self._rotate_api_key() # Attempt rotation even on failure for next file
                else:
                    ...
                    if wait_err:
                        ...
                        self._cleanup_specific_api_file(uploaded_file_obj.name) # Clean up the failed file
                    else:
                        # Generate analysis for the active file
                        self.message_queue.put({"type": "progress", "session_id": self.session_id, "filename": filename, "message": "Analyzing..."})
                        analysis_text, analysis_err = self._generate_analysis_for_file(active_file_obj, filename, is_chunk=False)
                        ...
                        if analysis_err:
                            ...
                            if "Invalid API Key" in analysis_err or "All API Keys Failed" in analysis_err:
                                self._rotate_api_key() # Attempt rotation even on failure for next file
                            # Don't cleanup API file here, _generate_analysis_for_file might retry
                        else:
            ...
            self.message_queue.put({
                ...
                "filename": filename,
                "status": final_status,
                "combined_analysis": combined_analysis # Send analysis even if partial/failed
            })

            # Signal end of processing for this file task
            try:
                self.message_queue.put(None) # Signal completion for this specific file task
            ...
            # Cleanup resources associated with this task
            self._cleanup_api_files() # Cleans all files tracked in self.uploaded_api_file_names
            self._cleanup_temp_folder() # Removes the session's temp folder

            # Remove the original input file from the temp location
            if os.path.exists(filepath):
                try:
                    # Ensure it's within the expected temp base to avoid accidental deletion
                    if self.task_temp_folder in os.path.abspath(filepath):
                         os.remove(filepath)
                         logger.info(f"Analyzer {self.session_id}: Removed local input file '{filepath}'")
                    else:
                         logger.warning(f"Analyzer {self.session_id}: Skipped removing '{filepath}' as it seems outside the task temp folder.")
                except OSError as rm_err:
                    logger.error(f"Analyzer {self.session_id}: Failed removing local input file '{filepath}': {rm_err}")
    ...
    def _generate_analysis_for_file(self, file_object, original_filename, is_chunk=False, chunk_num=0, total_chunks=0, previous_analysis_summary=""):
        generate_retries = 0
        last_error_type = "No Attempts Made"
        safety_settings = {
        ...
        if is_chunk:
            prompt = (FIRST_CHUNK_ANALYSIS_PROMPT if chunk_num == 1 else CHUNK_ANALYSIS_PROMPT).format(
            ...
            prompt = FILE_ANALYSIS_PROMPT.format(filename=api_display_name)
        contents = [prompt, file_object]
        while generate_retries <= MAX_GENERATE_RETRIES:
            if not self.current_api_key:
            ...
            try:
                ...
                logging.info(f"Analyzer {self.session_id}, File {log_filename_display}: Generating analysis (Attempt {generate_retries + 1}/{MAX_GENERATE_RETRIES + 1})")
                ...
                logger.exception(f"Analyzer {self.session_id}: Unexpected error generating {log_filename_display} (Attempt {generate_retries}/{MAX_GENERATE_RETRIES + 1})")
                if generate_retries > MAX_GENERATE_RETRIES:
                    error_payload = {"type": "file_error", "session_id": self.session_id, "filename": original_filename, "error": last_error_type}
                    if is_chunk: error_payload.update({"chunk_num": chunk_num, "total_chunks": total_chunks, "type": "chunk_error"})
                    self.message_queue.put(error_payload)
                    return None, last_error_type
    ...
    # --- Private Helper Methods: File Chunking ---
        ...
        try:
            ...
            if file_ext == '.pdf':
                ...
                if size_mb > PDF_CHUNK_SIZE_LIMIT_MB:
                    logger.info(f"Analyzer {self.session_id}: PDF '{filename}' ({size_mb:.2f} MB) requires chunking.")
                    return True, 'pdf'
    ...
    # --- Private Helper Methods: Cleanup ---
